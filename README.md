...
(This Table of Contents was manually created by myself.)
...
- Everything what you use from this project you do it at your own risk. I'll not resposible for any health or device damage.
- Further i'm not going to teach you how to install a LAMP stack on RPI. There are many how to's around the internet to successfully have this done.

If you need external services like ThingSpeak or Blynk you should use the original [Arduino Sketch](https://www.instructables.com/id/Solar-Powered-WiFi-Weather-Station-V20/) or clones around this project. For example:
...
If you want to use more than one device then rename the file to something like `config-one.h` and so on.
Declare the correct one in `solartherm-wifi-local-admin.ino` (comment out existing others declared there). You can only have one device to build the binary.
...
1. Copy the content of admin folder directly to the location on your LAMP stack web root or subfolder.
2. Create a API key with PHP on your webroot with PHP password_hash function like for example: echo $hash = password_hash("S47TnP3pp3R", PASSWORD_DEFAULT);
  - `S47TnP3pp3R` should a self defined salt what you have to set inside `remote-temperature.php`
  - The created password hash you have to enter in `config.h`
...
That's it! In every measurement cycle SolarTherm will push the data to your local netwotk storage.
...
3. In SolarTherm's `config.h` specify:
  ...
  - sensorId to identify device on MagicMirror module config `MAGIC_MIRROR_SENSORID` (multiple definitions are possible)
...
In this project only the HTTP Update from OTA is avaiable because all other solutions are not suitable due to the deep sleep functionality.
...
- webspace at desired webhoster with access to Apache, PHP and r/w permissions
- familiar with binary build on Arduino IDE
...
1. Customize the parameters in `config.h`:
  - URL of update host `OTA_UPDATE_HOST` (could also IP address or dyndns)
  - Port is used `OTA_UPDATE_PORT`
  - Path on you webroot where the update script is located `OTA_UPDATE_SCRIPT_PATH`
  - defines the name for your device `OTA_UPDATE_VERSION` to identifiy the correct *.bin sketch file
2. Define the MAC address of your devices inside array config `ota-update.php` located in solartherm-ota folder with the name you defined in `config.h`
3. Build the device binary with Arduino IDE name the file you defined under `OTA_UPDATE_VERSION`
4. Transfer the file to your webroot defined in step 1 for example: `solartherm-ota/bin` folder
...
Every time you build a new binary a new md5 checksum for your compiled file. You upload the file to your webroot.
If a device is awaking from deep sleep the device transfers with the script call header information.
This header have special content like the MAC address and md5 checksum of installed firmware.
